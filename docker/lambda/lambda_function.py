...
def lambda_handler(event, context):
    ...
    model_kwargs = body.get('model_kwargs')
    metadata = body.get('metadata')
    memory_window = body.get('memory_window')
    ...
    if region == 'ap-northeast-1' and bedrock_model_id == 'anthropic.claude-3-sonnet-20240229-v1:0':
    ...
    return {
        'statusCode': 200,
        'body': json.dumps({"question": prompt.strip(), "answer": generated_text.strip(), "documents": doc_url})
...
def init_conversationchain(session_id,region, bedrock_model_id, model_kwargs, metadata, memory_window, host) -> ConversationalRetrievalChain:
    ...
    if metadata == "NA":
        ...
        print('everyone',retriever)
    else:
        # retriever = new_db.as_retriever(search_kwargs={"filter": [{"term": {"metadata.year": metadata}}]})
        retriever = new_db.as_retriever(search_kwargs={"filter": [{"terms": {"metadata.acl.allowed": [metadata]}}]})
        print('sid',retriever)
