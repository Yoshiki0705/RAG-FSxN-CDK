...
export interface SecurityConstructProps {
  ...
  projectName: string;
  environment: string;
  namingGenerator?: any; // Agent Steering準拠命名規則ジェネレーター（オプション）
}
...
export class SecurityConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string, private props: SecurityConstructProps) {
    super(scope, id);
   ...
   * WAF WebACL作成
   */
  private createWafWebAcl(): wafv2.CfnWebACL | undefined {
   ...
   */
  private createGuardDutyDetector(): guardduty.CfnDetector | undefined {
    // 設定による制御
    if (!this.props.config.guardDuty.enabled) {
      cdk.Annotations.of(this).addInfo(
        'GuardDuty Detector: 設定により無効化されています。' +
    ...
    cdk.Annotations.of(this).addWarning(
      'GuardDuty Detector: 既存のDetectorとの競合を避けるため一時的に無効化されています。' +
   ...
   * IAM設定
   */
  private configureIamSettings(): void {
   ...
   * 出力値作成
   */
  private createOutputs(): SecurityOutputs {
    return {
   ...
   */
  private addCloudTrailBucketPolicies(bucket: s3.Bucket): void {
    // 定数定義による保守性向上
   ...
   */
  private createCloudTrailPolicy(config: {
    sid: string;
    actions: string[];
    resources: string[];
    ...
    const policyConfig: iam.PolicyStatementProps = {
      sid: config.sid,
