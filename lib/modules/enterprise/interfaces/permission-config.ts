...
export interface TimeBasedRestriction {
  ...
  readonly businessHours: {
    /** 開始時刻（0-23の範囲） */
    readonly startHour: number; // 0 <= startHour <= 23
    /** 終了時刻（0-23の範囲） */
    readonly endHour: number; // 0 <= endHour <= 23
    /** 営業日（0=日曜日, 1=月曜日, ..., 6=土曜日） */
    readonly businessDays: number[]; // 各要素は0-6の範囲
...
export interface DynamicPermissionConfig {
  ...
  /** 権限更新頻度（秒、最小値: 60秒） */
  readonly refreshIntervalSeconds: number; // >= 60
