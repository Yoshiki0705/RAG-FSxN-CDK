...
import { APIConfig, APIOutputs, APIType } from '../interfaces/api-config';
...
export class APIConstruct extends Construct {
  ...
  public readonly usagePlans: { [planName: string]: apigateway.UsagePlan } = {};
  public readonly apiKeys: { [keyName: string]: apigateway.ApiKey } = {};
  public readonly monitoringDashboard: cloudwatch.Dashboard;
  
  // マルチテナント関連
  public readonly tenantManagerFunction?: lambda.Function;

  constructor(scope: Construct, id: string, private props: APIConstructProps) {
    super(scope, id);
  ...
  private createGraphQLApi(): appsync.GraphqlApi {
    ...
    Object.entries(graphqlConfig.resolvers).forEach(([typeName, fields]) => {
      Object.entries(fields).forEach(([fieldName, config]) => {
        let dataSource: appsync.BaseDataSource;
   ...
   * WebSocket API作成
   */
  private createWebSocketApi(): apigatewayv2.WebSocketApi {
   ...
   * API分析設定
   */
  private setupAPIAnalytics(): void {
    ...
    if (analyticsConfig.kinesis.enabled) {
      this.kinesisStream = new kinesis.Stream(this, 'APIAnalyticsStream', {
        ...
        retentionPeriod: Duration.days(analyticsConfig.kinesis.retentionPeriod),
        shardCount: 1,
      });
    }
   ...
   * マルチテナント管理関数作成
   */
  private createTenantManagerFunction(): lambda.Function {
...
*
   ...
   * 出力値作成
   */
  private createOutputs(): APIOutputs {
    return {
      // Cognito出力
      ...
      usagePlanIds: Object.fromEntries(
        Object.entries(this.usagePlans).map(([name, plan]) => [name, plan.usagePlanId])
      ),
      apiKeyIds: Object.fromEntries(
        Object.entries(this.apiKeys).map(([name, key]) => [name, key.keyId])
      ),
  ...
  private applyTags(): void {
    ...
    Tags.of(this).add('APIType', tags.APIType);
    Tags.of(this).add('AuthProvider', tags.AuthProvider);
    Tags.of(this).add('CDNProvider', tags.CDNProvider);
    if (tags.SecurityLevel) {
      Tags.of(this).add('SecurityLevel', tags.SecurityLevel);
    }
    if (tags.CacheStrategy) {
      Tags.of(this).add('CacheStrategy', tags.CacheStrategy);
    }
  }
